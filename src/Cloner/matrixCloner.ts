 ...
 * @param size The distance from one clone to another in x,y and z direction. Default { x: 2, y: 2, z: 2 }.
    ...
    for (let z = start; z < this._mcount.z; z++) {
      for (let y = start; y < this._mcount.y; y++) {
        for (let x = start; x < this._mcount.x; x++) {
          const n = new CMesh(`n_lc${MatrixCloner.instance_nr}_${x}${y}${z}`, this._scene, this._rootNode);
          this._clones.push(n);
          const xyz = x + this._mcount.x * y + this._mcount.x * this._mcount.y * z;
    ...
    for (let z = 0; z < this._mcount.z; z++) {
      for (let y = 0; y < this._mcount.y; y++) {
        for (let x = 0; x < this._mcount.x; x++) {
          ...
          (this._clones[xyz].getChildren()[0] as Mesh).position = this.ePosition(Cloner.vZero);
