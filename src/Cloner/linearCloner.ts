...
export class LinearCloner extends Cloner {
  ...
  constructor(
    ...
    this._countNumberGen = (count as any) instanceof RandomNumberGen ? count : null;

    this._count = (count as any) instanceof RandomNumberGen ? (count as any).nextInt() : Number(count);

    this._offset = offset;

    this._P = new Vector3(P.x, P.y, P.z);
    this._S = new Vector3(S.x, S.y, S.z);
    this._R = new Vector3((R.x * Math.PI) / 180, (R.y * Math.PI) / 180, (R.z * Math.PI) / 180);
  ...
  createClone(parent: CMesh) {
    const cnt = this._countNumberGen != null ? (this._countNumberGen as any).nextInt() : this._count;
  ...
  private createClones(start = 0) {
    for (let i = start; i < this._count!; i++) {
      ...
      const n = new CMesh(`n_lc${this._instance_nr}_${i}`, this._scene, this._rootNode);
      this._clones.push(n);
  ...
  private calcSize() {
    for (let i = 1; i < this._count!; i++) {
      const orig = Vector3.Lerp(Cloner.vOne, this._S, this._iModeRelative ? i : i / (this._count! - 1));
      (this._clones[i].getChildren()[0] as Mesh).scaling = this.eScale(orig);
    }
  }
  ...
  private calcRot() {
    for (let i = 1; i < this._count!; i++) {
      const vRot = Vector3.Lerp(
        Cloner.vZero,
        this._R,
        this._iModeRelative ? i * this._growth : (i / (this._count! - 1)) * this._growth
      );
      (this._clones[i].getChildren()[0] as Mesh).rotation = this.eRotate(vRot);
    }
  }
