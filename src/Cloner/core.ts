...
export class CMesh extends Mesh {
  ...
  constructor(name: string, scene: Scene, parent: Mesh | null, cloner: Cloner | null = null) {
    super(name, scene, parent);
    this._cloner = cloner;
    this.parent = parent;
  }

  createClone(item: Cloner | Mesh, useInstances: boolean, name: string, isPickable: boolean) {
...
export class Cloner {
  ...
  eScale(vec: Vector3): Vector3 {
    ...
    for (let i = 0; i < this._effectors.length; i++) {
      vRet = Vector3.Lerp(vec, this._effectors[i].effector.updateScale(vRet), this._effectors[i].sensitivity);
    }
    return vRet;
  }
  eRotate(vec: Vector3): Vector3 {
    let vRet = Cloner.vZero.add(vec);
    for (let i = 0; i < this._effectors.length; i++) {
      vRet = Vector3.Lerp(vec, this._effectors[i].effector.updateRotation(vRet), this._effectors[i].sensitivity);
    }
    return vRet;
  }
  ePosition(vec: Vector3): Vector3 {
    let vRet = Cloner.vZero.add(vec);
    for (let i = 0; i < this._effectors.length; i++) {
      vRet = Vector3.Lerp(vec, this._effectors[i].effector.updatePosition(vRet), this._effectors[i].sensitivity);
    }
    return vRet;
  }
  eReset() {
