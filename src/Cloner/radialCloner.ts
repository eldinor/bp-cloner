...
export class RadialCloner extends Cloner {
  ...
  createClones(start = 0) {
    for (let i = start; i < this._count!; i++) {
      ...
      const n = new CMesh(`n_rc${this._instance_nr}_${i}`, this._scene, this._rootNode);
      n._index = i;
      this._clones.push(n);
    ...
    for (let i = 0; i < this._count!; i++) {
      ...
      (this._clones[i].getChildren()[0] as Mesh).rotation.x =
        ...
        (this._clones[i].getChildren()[0] as Mesh).rotation.z =
          0;
      if (this._plane.y === 0) {
        (this._clones[i].getChildren()[0] as Mesh).rotation.y = this._align
          ? this._offset + this._startangle + i * step
          : 0;
      } else if (this._plane.x === 0) {
        (this._clones[i].getChildren()[0] as Mesh).rotation.x = this._align
          ? -this._offset - this._startangle - i * step
          : 0;
      } else {
        (this._clones[i].getChildren()[0] as Mesh).rotation.z = this._align
          ? -this._offset - this._startangle - i * step
          : 0;
      }

      const vRet = this.eRotate((this._clones[i].getChildren()[0] as Mesh).rotation);
  ...
  private calcSize() {
    for (let i = 0; i < this._count!; i++) {
      (this._clones[i].getChildren()[0] as Mesh).scaling = this.eScale(Cloner.vOne);
    }
  }
